...
class CrawlerService:
        ...
        for tr in infobox.find_all("tr"):
            ...
            if th and "Directed by" in th.get_text():
                ...
                if link:
                    ...
                    # Fallback: lấy dòng đầu, bỏ ký tự thừa
    ...
    def _get_or_create_director(self, name: str, birth_year) -> Director:
        """
        Tra cache → tra DB → tạo mới.
        Cập nhật birth_year nếu trước đó chưa có.
        """
    ...
    def crawl_movies(self, limit: int = 50) -> int:
        ...
        # Duyệt hết bảng, dừng khi đủ limit
        for row in rows:
            if count >= limit:
                break
