import matplotlib
matplotlib.use("Agg")  # Phải đặt trước khi import pyplot
...
class AnalysisService:
    ...
    def generate_charts(self, director_id=None):
        ...
        # ── Dữ liệu biểu đồ 1 & 4: doanh thu + số lượng theo năm ─────
        year_revenue = {}
        year_count = {}
        for m in movies:
            if m.revenue:
        ...
        # ── Dữ liệu biểu đồ 3: doanh thu theo đạo diễn ──────────────
        director_data = {}
        for m in movies:
            if not m.revenue:
                continue
        ...
        # ── Dữ liệu biểu đồ 5: histogram phân bố doanh thu ───────────
        all_revenues_b = [m.revenue / 1_000_000_000 for m in movies if m.revenue]

        # ── Dữ liệu biểu đồ 6: doanh thu trung bình theo thập kỷ ─────
        decade_revenue = {}
        decade_count   = {}
        for m in movies:
            if not m.revenue:
        ...
        # Biểu đồ 1 — Line chart xu hướng doanh thu theo năm
        ax1.plot(
        ...
        # Biểu đồ 2 — Bar chart top 5 phim
    ...
    def export_chart_png(self, file_path: str, director_id=None) -> bool:
        """Lưu biểu đồ ra file PNG. Trả về True nếu thành công."""
        try:
